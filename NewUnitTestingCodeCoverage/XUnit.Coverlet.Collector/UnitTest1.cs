...
public class UnitTest1
{
    ...
    public void СравниваютсяДваКорняКратностиОдин(string x1, string x2)
    {
        ...
        Assert.True((Math.Abs(this.solution[0] - expected[0]) < eps)&&(Math.Abs(this.solution[1] - expected[1]) < eps));
    }
    ...
    {
        var expected = 1;
        Assert.True(Math.Abs(this.solution[0] - expected) < eps);
    }

    [Then(@"множество корней квадратного уравнения пустое")]
    public void СравниваетсяСПустымМножествомКорней()
    {
        Assert.True(this.solution.Length == 0);
    }
}